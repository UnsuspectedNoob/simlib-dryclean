...
void list_file(int option, int list) {
   ...
   * option = FIRST place at the start of list.
  ...
  if (list_size[list] == 1) {
    ...
    if ((option == INCREASING) || (option == DECREASING)) {
      ...
      /* Check to see if position if first or last. If so, take care of
       * it below. */
      if (row == head[list])
        option = FIRST;
...
void list_remove(int option, int list) {
   ...
   * Update timest statistics for the lsit.
...
void event_schedule(double time_of_event, int type_of_event) {
  ...
  /* printf("at time %lf, Event type %d scheduled at %lf\n\n", sim_time,
         type_of_event, time_of_event); */
...
int event_cancel(int event_type) {
  ...
  /* Check to see if this is the head of the list. If it is at the head, then it
   * MUST be a match. */
...
double sampst(double value, int variable) {
   ...
   * initialized accumulators > 0 updates sum, count, min, and max accumulators
...
void pprint_out(FILE *unit,
                int i) /* Write ith entry in transfer to file "unit". */
{
...
double expon(double mean,
             int stream) /* Exponentail variate generation function. */
{
  return -mean * log(lcgrand(stream));
}

int random_integer(float prob_distrib[],
                   int stream) /* Discrete-variate generation function. */
{
  int i;
  float u;
...
double uniform(double a, double b,
               int stream) /* Uniform variate generation function. */
{
  return a + lcgrand(stream) * (b - a);
}

double erlang(int m, double mean,
              int stream) /* Erlang variate generation function. */
{
  int i;
